"""Research-only NFL prop efficiency layer. No sportsbook binding or production authority."""
...
def build_efficiency_distribution(metric: str, pit_inputs: Mapping[str, Any]) -> EfficiencyDistribution:
    ...
    uncertainty = float(pit_inputs.get("uncertainty", 0.08 if "rate" in metric else 1.0))
    ...
    return EfficiencyDistribution("NFL", metric, mean, uncertainty)
