...
def fit_pit(rows: Iterable[DriveRow], *, prediction_cutoff_utc: str, source_manifest_sha256: str, feature_policy_sha256: str, code_sha: str) -> dict:
    ...
    starts=[]; possessions={}; game_scoring={}; off={}; deff={}
    for r in accepted:
    ...
    fit={
        ...
        "code_sha":code_sha,
        "training_rows":n,
        "minimum_team_drives_before_unshrunk_use":MIN_UNSHRUNK_DRIVES,
        "prior_drives":PRIOR_DRIVES,
        "league_scoring_rate":league_scoring,
        "base_params":{
            "drives_mean":mean_drives,"drives_sd":var_drives**0.5,
            "start_yard_mean":mean_start,"start_yard_sd":var_start**0.5,
            "shared_efficiency_sd":shared_sd,
            "outcome_probs":global_probs,"outcome_probs_by_start_bin":by_bin,
        },
...
def params_for_matchup(fit: Mapping, *, offense: str, defense: str, expected_source_manifest_sha256: str, expected_feature_policy_sha256: str, expected_code_sha: str) -> V2KParams:
    ...
    if fit.get("source_manifest_sha256")!=expected_source_manifest_sha256 or fit.get("feature_policy_sha256")!=expected_feature_policy_sha256 or fit.get("code_sha")!=expected_code_sha:
        raise ValueError("train/serve immutable identity mismatch")
    if not offense or not defense:
        raise ValueError("offense and defense required")
    ...
    league=float(fit["league_scoring_rate"])
    target=_sigmoid(_logit(league)+delta)
    scale=target/league if league>0 else 1.0
    scoring={DriveOutcome.TD.value,DriveOutcome.FG.value}
    probs=dict(base["outcome_probs"])
    for k in probs:
    ...
    return V2KParams(
        drives_mean=float(base["drives_mean"]), drives_sd=float(base["drives_sd"]),
        start_yard_mean=float(base["start_yard_mean"]), start_yard_sd=float(base["start_yard_sd"]),
        shared_efficiency_sd=float(base["shared_efficiency_sd"]),
        outcome_probs=probs, outcome_probs_by_start_bin=by_bin,
    )
