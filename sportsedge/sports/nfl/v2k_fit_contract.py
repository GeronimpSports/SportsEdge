...
def fit_pit(rows: Iterable[DriveRow], *, prediction_cutoff_utc: str, source_manifest_sha256: str, code_sha: str) -> dict:
    ...
    n=len(accepted); mean_start=sum(starts)/n
...
def params_from_fit(fit: Mapping, *, expected_source_manifest_sha256: str, expected_code_sha: str, shared_efficiency_sd: float=0.0) -> V2KParams:
    ...
    p=fit["params"]
    return V2KParams(drives_mean=float(p["drives_mean"]),drives_sd=float(p["drives_sd"]),start_yard_mean=float(p["start_yard_mean"]),start_yard_sd=float(p["start_yard_sd"]),shared_efficiency_sd=float(shared_efficiency_sd),outcome_probs=dict(p["outcome_probs"]))
