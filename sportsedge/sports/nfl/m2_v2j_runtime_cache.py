...
def market_readout_exact_cached(model: Any, row: dict[str, Any]) -> dict[str, Any]:
    ...
    # Safety PMFs depend only on the possession count and the already-computed
    # game-level safety probability, never on the shared environment.
    home_safety = {
    ...
    # One mask bundle per empirical possession-regime support.  The score support
    # keys are structural; probabilities change with environment but keys do not.
    ...
    for env, env_weight in model.shared_environment:
        ...
        # Repeated convolution is deterministic for one conditioned PMF/count.
        # Reusing it for another regime with the same count is byte-equivalent to
        # recomputing the identical nested loops.
        ...
        for home_drives, away_drives, regime_weight in regimes:
            ...
            # Keep these floating operations in the exact reference order.
