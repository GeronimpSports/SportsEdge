"""First frozen historical readout for preregistered research-only NFL V2I.

No V2I parameter may be changed based on this readout without a new
...
def _market_readout(model, row: dict[str, Any]) -> dict[str, Any]:
    ...
    spread_win = spread_push = 0.0
    total_win = total_push = 0.0
    total_weight = 0.0
    key_profile = {str(key): 0.0 for key in _KEYS}

    for home_drives, away_drives, regime_weight in model.possession_regime:
        home_scores = _combine_pmfs(
            _repeat_convolution(home_drive, home_drives),
            _safety_pmf(away_drives, home_safety_p),
        )
        away_scores = _combine_pmfs(
            _repeat_convolution(away_drive, away_drives),
            _safety_pmf(home_drives, away_safety_p),
        )
        ...
        weights = float(regime_weight) * np.outer(home_probs, away_probs)
        margins = home_values[:, None] - away_values[None, :]
        totals = home_values[:, None] + away_values[None, :]
        regime_total = float(weights.sum())
        total_weight += regime_total
    ...
    if not isfinite(total_weight) or abs(total_weight - 1.0) > 1e-9:
        raise ValueError("NFL_M2_V2I_WEIGHT_CONSERVATION_FAILED")

    candidate_home = None
    if spread_line is not None:
        active = total_weight - spread_push
        candidate_home = None if active <= 0 else _clip(spread_win / active)
    candidate_over = None
    if total_line is not None:
        active = total_weight - total_push
        candidate_over = None if active <= 0 else _clip(total_win / active)
...
def build_nfl_m2_v2i_raw_evaluations(
    ...
    *,
    ...
    for fold in folds:
        model = fit_nfl_m2_v2i_candidate(fold.train_rows, prior_drives=prior_drives)
        if int(fold.test_season) in model.train_seasons:
            raise ValueError("NFL_M2_V2I_TEST_SEASON_IN_TRAINING")
        for raw in fold.test_rows:
            ...
            if home_score is None or away_score is None:
                raise ValueError("NFL_M2_V2I_REALIZED_SCORE_MISSING")
...
def _loss(pairs: list[tuple[int, float]]) -> tuple[float, float]:
    if not pairs:
        raise ValueError("NFL_M2_V2I_EMPTY_EVALUATION")
    ll = brier = 0.0
    ...
    for season in sorted({int(row["season"]) for row in evaluations}):
        ...
        for market, (outcome_key, baseline_key, candidate_key) in specs.items():
            ...
            output.append({
                ...
                "candidate_probability_source": "V2I_MARKET_BLIND_SHARED_GAME_REGIME_PLUS_FOLD_SAFE_ISOTONIC",
            })
    return output
...
def _profile(raw: list[dict[str, Any]]) -> dict[str, Any]:
    if not raw:
        raise ValueError("NFL_M2_V2I_PROFILE_EMPTY")
    ...
    for row in raw:
        ...
        for key in _KEYS:
            ...
            if not 0.0 <= value <= 1.0:
                raise ValueError("NFL_M2_V2I_KEY_PROBABILITY_INVALID")
            totals[key] += value
    n = float(len(raw))
    return {
        "contract": "NFL_M2_V2I_OOS_SIGNED_KEY_PMF_V1",
        "model_id": NFL_M2_V2I_CANDIDATE_MODEL_ID,
        "distribution_contract": NFL_M2_V2I_DISTRIBUTION_CONTRACT,
        "event_contract": NFL_M2_V2H_EVENT_CONTRACT,
        "probability_source": "FIRST_OOS_PREREGISTERED_V2I_SHARED_GAME_REGIME_DISTRIBUTION",
...
def build_nfl_m2_v2i_candidate_evidence(
    rows: Iterable[dict[str, Any]],
    *,
    ...
    if len(manifest) != 64:
        raise ValueError("NFL_M2_V2I_SOURCE_MANIFEST_SHA256_INVALID")
    try:
        int(manifest, 16)
    except ValueError as exc:
        raise ValueError("NFL_M2_V2I_SOURCE_MANIFEST_SHA256_INVALID") from exc

    raw = build_nfl_m2_v2i_raw_evaluations(
        rows,
