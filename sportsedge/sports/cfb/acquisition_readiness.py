...
This module performs no network calls. It verifies the frozen model-selection
policy's acquisition-start conditions before any CFBD replay request is allowed.
Unknown account tier/quota/cache facts block rather than being guessed.
"""
...
class CFBAcquisitionReadinessError(ValueError):
    pass
...
def _cache_blockers(cache: object) -> list[str]:
    ...
    for index, raw in enumerate(cache):
        ...
        if _nonempty(endpoint) and season is not None and _nonempty(provider):
            identity = (str(endpoint), season, end_week, str(provider))
            if identity in seen:
...
def audit_cfb_acquisition_readiness(
    ...
    """Audit whether a historical replay may begin without making any request."""
    blockers: list[str] = []
    gate = policy.get("acquisition_start_gate")
    ...
    cache_blockers = _cache_blockers(acquisition.get("cache_manifest"))
    blockers.extend(cache_blockers)

    cache_count = len(acquisition.get("cache_manifest") or []) if isinstance(acquisition.get("cache_manifest"), list) else 0
    ready = not blockers
    return {
        ...
        "verified_cache_entries": cache_count,
...
__all__ = ["CFBAcquisitionReadinessError", "audit_cfb_acquisition_readiness"]
