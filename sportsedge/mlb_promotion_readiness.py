...
def build_mlb_promotion_readiness(
    ...
    **finish_line_kwargs: Any,
    ...
    ``acceptance_complete`` remains the existing evidence-gate result. A frozen
    edge floor is an additional production prerequisite. ``official_ready`` is
    intentionally stricter still and requires the deployment registry to already
    mark the market eligible; this function never changes that flag.
    """
    ...
    for raw in finish["markets"]:
        ...
        six_gate_complete = bool(row.get("acceptance_complete"))
        ...
        promotion_prerequisites_complete = bool(six_gate_complete and floor_frozen)
    ...
    return {
        ...
        "six_gate_complete_count": sum(bool(r["acceptance_complete"]) for r in rows),
