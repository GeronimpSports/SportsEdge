...
def raw_implied_probability(odds:int)->float:
    d=american_to_decimal(odds); return 1.0/d
...
class BoundProp:
    ...
    offered_odds:int; decimal_odds:float; expected_value_per_unit:float
...
def bind_prop_market(*,sport:str,market:str,entity_id:str,model_probability:float,offered_odds:int,validation_passed:bool,line:float|None=None,paired_other_side_odds:int|None=None)->BoundProp:
    ...
    if paired_other_side_odds is None:
        nv:float|str=NO_VIG_ONE_SIDED
    else:
        nv,_=proportional_devig(int(offered_odds),int(paired_other_side_odds))
    return BoundProp(sport,market,entity_id,line,p,int(offered_odds),dec,ev,nv,True)
