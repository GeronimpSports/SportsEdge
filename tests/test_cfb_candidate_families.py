...
class TestCFBCandidateFamilies(unittest.TestCase):
    ...
    def test_week2_plus_requires_exact_prior_week_current_season_metrics(self):
        ...
        out = materialize_candidate_row(FAMILY_EQUAL_WEIGHT_HARD_SWITCH, row)
        self.assertEqual(out, row)
        row["home_metrics"]["through_week"] = 2
        with self.assertRaisesRegex(
            CFBCandidateFamilyError, "CURRENT_SEASON_SWITCH_INVALID"
        ):
    ...
    def test_week1_requires_immediately_prior_season_fallback(self):
        ...
        row["home_metrics"] = metrics(
            season=2024, through_week=99, sample_source="PRIOR_SEASON_FALLBACK"
        )
        row["away_metrics"] = metrics(
            season=2024, through_week=99, sample_source="PRIOR_SEASON_FALLBACK"
        )
        self.assertEqual(
            materialize_candidate_row(FAMILY_EQUAL_WEIGHT_HARD_SWITCH, row), row
        )
        row["away_metrics"]["season"] = 2023
        with self.assertRaisesRegex(
            CFBCandidateFamilyError, "WEEK1_PRIOR_SEASON_SWITCH_INVALID"
        ):
            materialize_candidate_row(FAMILY_EQUAL_WEIGHT_HARD_SWITCH, row)
    ...
    def test_market_data_is_rejected(self):
        ...
        row["spread"] = -3.5
        with self.assertRaisesRegex(
            CFBCandidateFamilyError, "MARKET_DATA_PROHIBITED"
        ):
            materialize_candidate_row(FAMILY_EQUAL_WEIGHT_HARD_SWITCH, row)

    def test_baseline_disallows_hidden_tuning_constants(self):
        with self.assertRaisesRegex(
            CFBCandidateFamilyError, "BASELINE_CONSTANTS_PROHIBITED"
        ):
            materialize_candidate_row(
                FAMILY_EQUAL_WEIGHT_HARD_SWITCH,
                self.base_row(),
                constants={"weight": 0.75},
            )

    def test_other_predeclared_families_still_fail_closed(self):
        for family in (
            "RELIABILITY_WEIGHTED_HARD_SWITCH",
            "PRIOR_CURRENT_BLEND",
            "GAMES_IN_SAMPLE_FEATURE",
        ):
            with self.assertRaisesRegex(
                CFBCandidateFamilyError, "FAMILY_UNIMPLEMENTED"
            ):
