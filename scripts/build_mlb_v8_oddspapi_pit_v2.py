...
def attestation(root, fixture_path, fixture):
    ...
    checks=[a.get('schema')==ATT_SCHEMA,a.get('source')=='MLB_STATSAPI_GAME_FEED',a.get('promotion_authority') is False,a.get('retroactive_point_in_time_claim') is False,str(a.get('fixture_id'))==str(fixture.get('fixtureId')),a.get('fixture_sha256')==sha(fixture_path.read_bytes()),bool(a.get('game_feed_payload_sha256'))]
...
def build(root:Path,out:Path):
    ...
    summary={'schema':'MLB_V8_ODDSPAPI_PIT_ARCHIVE_V2','actual_start_required':True,'same_book_same_threshold_required':True,'rows':len(rows),'counts':dict(counts),'failures':failures,'promotion_eligible':False,'promotion_reason':'PAIRED_PRICE_EVIDENCE_ALONE_DOES_NOT_SATISFY_MODEL_REPLAY_HOLDOUT_CALIBRATION_OR_TRUTH_GATE'}
...
def self_test():
    ...
    print(json.dumps({'status':'SELF_TEST_OK','actual_start_required':True,'promotion_authority':False})); return 0
