#!/usr/bin/env python3
"""Validate first-readout V2H emergent key mass against the frozen NFL target."""
...
def main() -> int:
    ...
    if float(args.max_abs_error) != 0.005:
        raise SystemExit("NFL_V2H_FROZEN_KEY_TOLERANCE_REQUIRED")
    ...
    if evidence.get("status") != "FIRST_READOUT_DIAGNOSTIC_ONLY":
        raise SystemExit("NFL_V2H_FIRST_READOUT_STATUS_REQUIRED")
    ...
    for field in ("promotion_eligible", "promotion_authority", "model_p_authority", "official_status_granted"):
        if evidence.get(field) is not False:
            raise SystemExit(f"NFL_V2H_AUTHORITY_FORBIDDEN:{field}")
    if evidence.get("model_id") != NFL_M2_V2H_CANDIDATE_MODEL_ID:
        raise SystemExit("NFL_V2H_MODEL_ID_MISMATCH")
    if evidence.get("distribution_contract") != NFL_M2_V2H_DISTRIBUTION_CONTRACT:
        raise SystemExit("NFL_V2H_DISTRIBUTION_CONTRACT_MISMATCH")

    profile_payload = evidence.get("candidate_distribution_profile")
    if not isinstance(profile_payload, dict) or profile_payload.get("contract") != "NFL_M2_V2H_OOS_SIGNED_KEY_PMF_V1":
        raise SystemExit("NFL_V2H_PROFILE_INVALID")
    raw_pmf = profile_payload.get("signed_key_probability")
    if not isinstance(raw_pmf, dict):
        raise SystemExit("NFL_V2H_PMF_REQUIRED")

    profile = build_nfl_simulator_profile(audit, version="nfl-v2h-first-readout-oos-key-emergent-v1")
    ...
    try:
        ...
        raise SystemExit("NFL_V2H_PMF_INVALID") from exc
