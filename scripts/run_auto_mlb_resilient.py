...
from sportsedge.mlb_source import capture_mlb_source_bytes
...
def main() -> int:
    ...
    capture_root = Path(
        os.environ.get("SPORTSEDGE_MLB_PIT_SOURCE_DIR", "artifacts/mlb_pit_sources")
    ) / now.strftime("%Y%m%dT%H%M%S.%fZ")

    infrastructure_blocked = False
    with capture_mlb_source_bytes(capture_root) as pit_source_capture:
        try:
            common = dict(
    ...
    # This sidecar is descriptive provenance only. It cannot grant promotion,
    # eligibility, floors, or OFFICIAL status. Future V8 evidence may bind these
    # exact files only when the game/timestamp requirements also pass.
    payload["pit_source_capture"] = pit_source_capture.manifest()
