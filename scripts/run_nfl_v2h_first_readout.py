#!/usr/bin/env python3
"""Run the first preregistered historical readout for NFL V2H."""
...
from sportsedge.sports.nfl.m2_v2h_validation import build_nfl_m2_v2h_candidate_evidence
...
def main() -> int:
    ...
    if args.start_season != 2016 or args.end_season != 2025:
        raise SystemExit("NFL_V2H_FIRST_READOUT_FROZEN_WINDOW_REQUIRED")

    manifest, expected, manifest_sha = _load_manifest(args.source_manifest)
    if manifest_sha != manifest_sha256(manifest):
        raise SystemExit("NFL_V2H_MANIFEST_HASH_MISMATCH")
    ...
    evidence = build_nfl_m2_v2h_candidate_evidence(
        event_rows,
