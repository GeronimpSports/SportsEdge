#!/usr/bin/env python3
"""Compare research-only NFL V2G OOS key mass to the frozen historical target."""
...
_PROFILE_CONTRACT = "NFL_M2_V2G_OOS_SIGNED_KEY_PMF_V1"
...
def main() -> int:
    ...
    if evidence.get("status") != "DIAGNOSTIC_CANDIDATE_ONLY":
        raise SystemExit("NFL_V2G_DIAGNOSTIC_STATUS_REQUIRED")
    if evidence.get("promotion_eligible") is not False or evidence.get("promotion_authority") is not False:
        raise SystemExit("NFL_V2G_PROMOTION_AUTHORITY_FORBIDDEN")
    if evidence.get("model_id") != NFL_M2_V2G_CANDIDATE_MODEL_ID:
        raise SystemExit("NFL_V2G_MODEL_ID_MISMATCH")
    if evidence.get("distribution_contract") != NFL_M2_V2G_DISTRIBUTION_CONTRACT:
        raise SystemExit("NFL_V2G_DISTRIBUTION_CONTRACT_MISMATCH")

    profile_payload = evidence.get("candidate_distribution_profile")
    if not isinstance(profile_payload, dict) or profile_payload.get("contract") != _PROFILE_CONTRACT:
        raise SystemExit("NFL_V2G_CANDIDATE_PROFILE_INVALID")
    raw_pmf = profile_payload.get("signed_key_probability")
    if not isinstance(raw_pmf, dict):
        raise SystemExit("NFL_V2G_CANDIDATE_PMF_REQUIRED")

    profile = build_nfl_simulator_profile(audit, version="nfl-m2-v2g-candidate-oos-key-emergent-v1")
    ...
    try:
        ...
        raise SystemExit("NFL_V2G_CANDIDATE_PMF_INVALID") from exc
    if any(value < 0.0 or value > 1.0 for value in candidate_pmf.values()):
        raise SystemExit("NFL_V2G_CANDIDATE_PMF_INVALID")
